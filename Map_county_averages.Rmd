...
#Mao function
my_map.fn1<- function(shp, data, nclr, plotclr, Title){
...
my_map.fn2<- function(shp, data, nclr, plotclr, Title){
  ...
  class <- classIntervals(plotvar,
                          nclr,
                          style = "fixed", 
                          fixedBreaks = c(-5,-3,-2,-1,0,1,2,3,5,16))
  colcode <- findColours(class, plotclr)
...
par(mfrow=c(2,2),mai=c(1,1,0.5,0.5))
for (s in c('Spring','Summer','Autumn','Winter')){
  merged<- inner_join(as.data.frame(Countymap), df_plot[which(df_plot$season==s),], by = c("State_FIPS", "County_FIPS"))
